...
**第四步 — 定位 + 建議安排(PRD 4.2 / 4.3)**
- 地點可用名稱**定位**(免費 OpenStreetMap Nominatim)取得座標
- 行程分頁的 **✨ 建議安排**:把已定位地點依距離**自動分成 N 天 + 每天就近排序**
  (簡化版 k-means + 最近鄰;數學核心在 `geo.js`,可離線計算)
- 兩站之間顯示**距離 + 估計交通時間**(直線×係數的保底估算,之後可換真實路線 API)

### 後續步驟(往上疊,不打掉重練)

5. Google 地圖分享連結匯入(免手打)、地圖視覺化、真實路線 API
6. 旅途記錄(拍照歸位、記花費)
7. 旅遊書自動組裝 + AI 草稿
