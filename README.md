...
- 兩站之間顯示**多模式交通估時**:🚶步行 / 🚌巴士 / 🚇捷運 / 🚗駕車(粗估,之後可換真實路線 API)
- **✨ 建議安排**兩種方式:
  - **只優化每天順序(預設)**:保留使用者分好的天,只把每天景點依距離就近重排(不跨日移動)
  - **跨日重新分配**:把所有已定位景點依距離重新分到各天(需使用者主動選)
- **住宿當錨點**:住宿類地點可設為某天的**出發點 / 回程點**,和景點分開顯示;
  同一間可跨多天(連住),換飯店就設不同天;排序會從當天出發點起算
- 數學核心在 `geo.js`,可離線計算
